...
outVFileName = "gazeRecordings/aruco/gaze_objects_pos__remote.mp4"
...
sockClient.initialize_msgStruct(["clf_threshold", "img_dims", "img"])
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if sockClient.socketStream_ok():
                ...
                while msg is None:
                    msg = sockClient.get_latest()
                bboxes = np.array(msg['bboxes'])
                scores = np.array(msg['scores'])
                tt = np.array(msg['labels'])
            else:
                break
